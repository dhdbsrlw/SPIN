""" This code is copied and modified from /mDPO/seed_llama/SEED/models/seed_qformer/qformer_quantizer.py """
...
class Blip2QformerQuantizer(Blip2Base):
    ...
    def from_pretrained(cls, pretrained_model_path, **kwargs):
        ...
        # print("\n\n*** debug 1 ***")
        # print("model precision: ", model.Qformer.dtype)

        # print("\n\n*** debug 2 ***")
        # print("model device: ", model.Qformer.device)
        # # model.Qformer = model.Qformer.to('cuda').float() # modify
        # print("model after moving device: ", model.Qformer.device)


        # print("\n\n *** debug 0: vit_precision ***")
        # print(vit_precision)

        # print("\n\n *** debug 1 ***")
        # print(model.Qformer.bert.embeddings.LayerNorm)
        ...
        # Ensure the precision of your model and checkpoint is consistent. (10/18)
        ...
        if pretrained_model_path.startswith('http'):
            ...
            # 문제점: Weight precision: torch.bfloat16 in SPIN CODE

            print("\n\n *** debug 3 ***")
            print("Weight dtype:", type(model.Qformer.bert.embeddings.LayerNorm.weight))
            print("Weight precision:", model.Qformer.bert.embeddings.LayerNorm.weight.dtype) 
            # Weight precision: torch.float32
            # print("ckpt: ", ckpt.keys()) #Qformer.bert.embeddings.LayerNorm.weight.dtype)

        else:
            ckpt = torch.load(pretrained_model_path, map_location="cpu")

        # model = reinitialize_model_parameters_with_zeros(model, ckpt) # modify (WRONG)
        # ckpt = clean_ckpt_parameters(model, ckpt)

        # 드디어 성공 
        cleaned_model = clean_and_modify_model_parameters(model, ckpt)
        missing, unexcepted = cleaned_model.load_state_dict(ckpt, strict=False)
...
# FILTERING CODE
def clean_ckpt_parameters(model, ckpt):
    # checkpoint = torch.load(ckpt_path)
    ckpt_state_dict = ckpt.get('state_dict', ckpt)
    model_state_dict = model.named_parameters()
    # print(model_state_dict)

    # Create a dictionary to hold the cleaned state dict
    cleaned_state_dict = {}

    # Loop over model parameters
    for name, param in ckpt_state_dict.items(): # ckpt['state_dict'].named_parameters():
        for mname, mparam in model.named_parameters():
            if name == mname:
                if param.size() == mparam.size() and mparam.size() != torch.Size([0]):
                    cleaned_state_dict[name] = param  
                else:
                    print(f"Skipping parameter: {mname} with model param size {mparam.size()}")

    return cleaned_state_dict

def clean_and_modify_model_parameters(model, ckpt):
    # Load the state dict from the checkpoint
    ckpt_state_dict = ckpt.get('state_dict', ckpt)  # Get the checkpoint's state dict

    # Loop over the model parameters and the checkpoint state dict
    for name, param in ckpt_state_dict.items():
        # Check if the model has the parameter with the same name
        if name in model.state_dict():
            model_param = model.state_dict()[name]

            # If the sizes match, keep the original model parameter
            if param.size() == model_param.size() and model_param.size() != torch.Size([0]):
                # print(f"Keeping parameter {name} with size {model_param.size()}")
                continue
            else:
                # If sizes don't match, replace the model's parameter with a new one
                print(f"Replacing model parameter {name} with new zero-initialized parameter of size {param.size()}")
                # Update the model's parameter directly
                with torch.no_grad():
                    new_param = nn.Parameter(torch.zeros(param.size()))  # Create new parameter with correct size
                    # name: Qformer.bert.encoder.layer.11.output_query.LayerNorm.bias 
                    # getattr(model, name.split('.')[0])._parameters[name.split('.')[-1]] = new_param
                    set_nested_attr(model, name, new_param) 

    return model

def set_nested_attr(model, param_name, new_param):
    """
    Recursively navigate the module hierarchy to set a new parameter.
    """
    # Split the parameter name into parts (e.g., ['Qformer', 'bert', 'encoder', 'layer', '11', 'output_query', 'LayerNorm', 'bias'])
    attrs = param_name.split('.')

    # Navigate through the model layers/modules to reach the last module
    module = model
    for attr in attrs[:-1]:  # Traverse all but the last part
        if attr.isdigit():
            module = module[int(attr)]  # If it's an integer index (e.g., for layers), convert and access it
        else:
            module = getattr(module, attr)  # Otherwise, use getattr to navigate

    # Replace the parameter in the final module (e.g., LayerNorm.bias)
    module._parameters[attrs[-1]] = new_param